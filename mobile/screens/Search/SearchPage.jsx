  ...
  const handleSearch = (keyword) => {
    ...
    if (keyword.trim() === "") {
      ...
      if (searchCategory === "foods") {
        setFilteredFoods(
          foods.filter(
            (food) => regex.test(food.title) || regex.test(food.code)
          )
        );
        ...
        setFilteredRestaurants(
          restaurants.filter(
            (restaurant) =>
              regex.test(restaurant.title) || regex.test(restaurant.code)
          )
        );
      }
    }
    ...
    <SafeAreaView>
      <View style={{ backgroundColor: COLORS.primary, height: SIZES.height }}>
        ...
        >
          <View style={styles.searchContainer}>
            <View style={styles.searchWrapper}>
              <TextInput
                ...
                placeholder={`Search ${
                  searchCategory === "food(s)" ? "food(s)" : "restaurant(s)"
                }...`}
              />
            </View>
          ...
          >
            <TouchableOpacity onPress={() => setSearchCategory("foods")}>
              <Text
                style={{
                  color:
                    searchCategory === "foods" ? COLORS.primary : COLORS.gray,
                  fontFamily: "regular",
                }}
              >
                Foods
          ...
          ) : (searchCategory === "foods" && filteredFoods.length === 0) ||
            (searchCategory === "restaurants" &&
