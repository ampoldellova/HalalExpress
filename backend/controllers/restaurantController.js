...
module.exports = {
  ...
  pickupAvailability: async (req, res) => {
    ...
    try {
      ...
      res.status(200).json({
        status: true,
        message: "Availability for Pick-Up Successfully Toggled",
        pickup: restaurant.pickup,
      });
    } catch (error) {
      res
        .status(500)
        .json({ status: false, message: "Error Toggling Restaurant's Pickup" });
    }
  },
  ...
  deliveryAvailability: async (req, res) => {
    ...
    try {
      ...
      res.status(200).json({
        status: true,
        message: "Availability for Delivery Successfully Toggled",
        delivery: restaurant.delivery,
      });
    } catch (error) {
      res.status(500).json({
        status: false,
        message: "Error Toggling Restaurant's Delivery",
      });
    }
  },
  ...
  getRestaurant: async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ status: false, message: "Error Retrieving Restaurant" });
    }
  },
  ...
  getRestaurantsByOwner: async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({
        status: false,
        message: "Error fetching restaurants by owner",
        error: error.message,
      });
    }
  },
  ...
  editRestaurantDetails: async (req, res) => {
    try {
      if (req.files) {
        ...
        res
          .status(201)
          .json({ success: true, message: "Restaurant Details Updated" });
      } else {
