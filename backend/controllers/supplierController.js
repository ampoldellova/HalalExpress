...
module.exports = {
    ...
    try {
      ...
      if (!supplierStores.length) {
        return res.status(404).json({
          status: false,
          message: "No Store of Supplier found for this owner",
        });
      }
      ...
      res.status(500).json({
        status: false,
        message: "Error fetching Store by owner",
        error: error.message,
      });
    }
  },
  ...
  getSupplier: async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ status: false, message: "Error Retrieving Supplier" });
    }
  },
  ...
  editSupplierDetails: async (req, res) => {
    try {
      if (req.files) {
        ...
        res
          .status(201)
          .json({ success: true, message: "Supplier Details Updated" });
      } else {
