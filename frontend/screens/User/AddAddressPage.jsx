...
const AddAddressPage = () => {
  ...
  const useCurrentLocation = async () => {
    ...
    fetch(
      `https://api.opencagedata.com/geocode/v1/json?q=${latitude}+${longitude}&key=a153a349ad474d8bb67e62bf4dadfa04`
    )
      .then((response) => {
        if (!response.ok) {
          ...
          {({
            ...
            <View>
              <View
                style={[
                  styles.inputWrapper(
                    touched.coords?.address ? COLORS.secondary : COLORS.offwhite
                  ),
                  { height: "auto" },
                ]}
              >
